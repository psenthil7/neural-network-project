...
public class SimpleNeuralNetwork implements NeuralNetwork {
    ...
    public float[] predict(float[] input) {
        ...
        if (input.length != inputLayer.length) {
            throw new RuntimeException("Input length must match number of input neurons.");
        }

        // Step 2: Create an ArrayQueue for BFS traversal of the network.
        // Also, create a set to keep track of visited neurons.
        Queue<Neuron> traverser = new ArrayQueue<>();
        ...
        for (int i = 0; i < inputLayer.length; i++) {
            ...
            traverser.enqueue(inputLayer[i]);
        }
        ...
        while (!traverser.isEmpty()) {
            Neuron currNeuron = traverser.dequeue();
            List<Neuron> adjacentNeurons = network.get(currNeuron);
            for (int i = 0; i < adjacentNeurons.size(); i++) {
                Neuron adj = adjacentNeurons.getItem(i);
                if (!visited.contains(adj)) {
                    visited.add(adj);
                    traverser.enqueue(adj);
                }
                adj.setInput(currNeuron.compute(), currNeuron.getIndex());
            }
        }
        ...
        for (Neuron outputNeuron : outputLayer) {
            result[outputNeuron.getIndex()] = outputNeuron.compute();
        }
